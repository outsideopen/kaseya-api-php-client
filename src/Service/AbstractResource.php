...
class AbstractResource
{
	...
	private $stackParameters = array(
		...
		'$skip'       => ['type' => 'int', 'location' => 'query'],
		'$top'        => ['type' => 'int', 'location' => 'query'],
	);
	/** @var string $rootUrl */
	private $rootUrl;
	/** @var Client $client */
	...
	public function __construct(AbstractService $service, $serviceName, $resourceName, $methods)
	{
		$this->serviceName  = $serviceName;
		$this->resourceName = $resourceName;
		$this->client       = $service->client();
		$this->rootUrl      = $this->client->rootUrl();
	...
	public function call($name, $arguments, $expectedClass = null)
	{
		...
		foreach ($parameters as $key => $val) {
			if ($key != 'postBody' && !isset($method['parameters'][$key])) {
				...
				throw new \RuntimeException("($name) unknown parameter: '$key'");
			}
		}
		foreach ($method['parameters'] as $paramName => $paramSpec) {
			if (isset($paramSpec['required']) &&
				...
				throw new \RuntimeException("($name) missing required param: '$paramName'");
			}
...
//		}
	...
	public function createRequestUri($restPath, $params)
	{
		...
		if (count($uriTemplateVars)) {
			...
			$requestUrl        = $uriTemplateParser->expand($requestUrl, $uriTemplateVars);
		}
		if (count($queryVars)) {
